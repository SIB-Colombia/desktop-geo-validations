...
public class Client {
	...
	public static void main(String[] args) throws IOException {
		...
		if (in.exists() && !in.isDirectory()) {
			FileWriter out = new FileWriter(
					"./AreasMaritimas/output/areaMaritimasValidacion_"
							+ fecha.get(Calendar.DAY_OF_MONTH) + "-"
							+ (fecha.get(Calendar.MONTH) + 1) + "-"
							+ fecha.get(Calendar.YEAR) + ".txt");
			...
			System.out.println("File areaMaritimas.txt not found");
		}
	...
	public void start() {
		...
		try {
			initTime = System.currentTimeMillis();
			System.out.println("< Asking work" + new Date());
			while (reader.hasNext()) {
				String line = reader.nextLine();
				String[] splited = line.split("\t");
				records.add(new Record(Long.parseLong(splited[0]), Double
						.parseDouble(splited[1].replace(",", ".")), Double
						.parseDouble(splited[2].replace(",", ".")), splited[3]));
			}
			work();
	...
	private void work() {
		...
		try {
			shapeFileManager.getAreasFromCoordinates(records);
